...
```bash
# Single file
powershell -File scripts/extract_xel.ps1 -Path "system_health_0_xxx.xel" -Output events.json

# Multiple files (glob)
powershell -File scripts/extract_xel.ps1 -Path "system_health*.xel" -Output events.json

# With time filter
powershell -File scripts/extract_xel.ps1 -Path "*.xel" -Days 3 -Output events.json

# UNC path
powershell -File scripts/extract_xel.ps1 -Path "\\server\share\system_health*.xel" -Output events.json
```
...
```bash
# Console summary
node scripts/parse_xevent.js events.json

# With time filter + JSON output
node scripts/parse_xevent.js events.json --days 3 --json --output xevent_findings.json

# Cross-correlate with ERRORLOG findings
node scripts/parse_xevent.js events.json --errorlog errorlog_findings.json --json --output xevent_findings.json
```
...
```bash
node scripts/parse_xevent.js events.json --errorlog errorlog_7days_findings.json
```
